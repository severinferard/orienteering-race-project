          ...
          ><v-card>
            <v-data-table :headers="headers" :items="balises" hide-default-footer fixed-header height="300px">
              <template v-slot:[`item.valided`]="{ item }">
                <v-icon :color="item.valided ? 'green' : 'red'">{{ item.valided ? "mdi-checkbox-marked" : "mdi-close-box" }}</v-icon>
              </template>
  ...
  methods: {
    ...
    async loadData() {
      ...
      try {
        ...
        if (!data.rawPositions.length) throw "Le movuino n'a enregistré aucune données";
        if (!data.beacons.length) throw "Auncune balises enregistrées";

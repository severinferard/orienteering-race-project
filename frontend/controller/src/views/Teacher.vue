            ...
            <v-tabs-items v-model="beaconTab">
              <v-tab-item key="table">
                <div style="height: 300px; overflow: scroll">
                  <Plotly :data="beaconPlotData.data" :layout="beaconPlotData.layout" :display-mode-bar="false"></Plotly>
                </div>
              </v-tab-item>
              <v-tab-item key="plot">
                <v-data-table :headers="beaconTableheader" :items="beacons" hide-default-footer fixed-header height="300px">
                  ...
                  <v-toolbar>
                    <v-row justify="center" align="center">
                      <v-col cols="7">
                        <div>
                          <v-tabs fixed-tabs>
                            <v-tab @click="studentTabPlotType = 'time'" v-model="studentTabPlotType" key="time">Chrono</v-tab>
                            <v-tab @click="studentTabPlotType = 'avgSpeed'" v-model="studentTabPlotType" key="avgSpeed">Vitesse</v-tab>
                            <v-tab @click="studentTabPlotType = 'distance'" v-model="studentTabPlotType" key="distance">Distance</v-tab>
                            <v-tab @click="studentTabPlotType = 'beacons'" v-model="studentTabPlotType" key="beacons">Balises</v-tab>
                          </v-tabs>
                        </div>
                      </v-col>
                    </v-row>
                  </v-toolbar>
                  <Plotly :data="studentPlotData.data" :layout="studentPlotData.layout" :display-mode-bar="false"></Plotly>
                </div>
              </v-tab-item>
              <v-tab-item key="plot">
                <v-data-table :headers="studentTableHeader" :items="students" hide-default-footer fixed-header height="500px">
                  <template v-slot:[`item.time`]="{ item }"> {{ Math.floor(item.time / 60) }}:{{ getMins(item.time) }} </template>
                </v-data-table>
              </v-tab-item>
            </v-tabs-items>
          </v-card>
      ...
      <v-row>
        <v-col>
          <v-card class="pa-5">
            <v-row>
              <v-col>
                <v-btn @click="downloadExcel" :style="{ left: '50%', transform: 'translateX(-50%)' }" outlined>
                    Télécharger le résumé de la séance au format .XLSX (Excel)
                </v-btn>
              </v-col>
            </v-row>
          </v-card>
...
export default {
  ...
  computed: {
    studentPlotData: function() {
      ...
      let ret = {
        data: [
          {
            x: [...this.students].sort((a, b) => b[this.studentTabPlotType] - a[this.studentTabPlotType]).map((std) => std.id),
            y: [...this.students]
              .sort((a, b) => b[this.studentTabPlotType] - a[this.studentTabPlotType])
              .map((std) => std[this.studentTabPlotType]),
            marker: {
              color: [...this.students].sort((a, b) => b[this.studentTabPlotType] - a[this.studentTabPlotType]).map((std) => std.color),
            },
            type: "bar",
            // width: 1,
          },
  ...
  methods: {
    ...
    async downloadExcel() {
      const res = await axios.get(`/api/excel/session/${this.$route.params.session_id}/`, { responseType: "blob" });

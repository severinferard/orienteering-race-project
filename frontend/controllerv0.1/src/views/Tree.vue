    ...
    <v-app-bar app color="primary" dark>
      <v-toolbar-title>
        <router-link to="/">
          ...
          <router-link to="/tree">
            <h4 class="secondary--text text--lighten-1 font-weight-light text-decoration-underline px-4">Dashboard</h4>
          </router-link>
    ...
    <v-container fluid class="fill-height">
      <v-row class="fill-height pa-4">
        ...
        <v-col>
          ...
          <v-card
            v-else-if="nodeTypeMap[selected.id] == 'school'"
            class="pt-6"
            flat
          >
            <v-card-text class="text-center">
              ...
              <v-divider></v-divider>
              <h4 class="subtitle-1 my-5">
                {{ selected.classes.length }} classes enregistrées
              </h4>
              <v-toolbar color="#46c4a1" dark dense>
                <v-toolbar-title class="px-3">Classes</v-toolbar-title>
                <v-btn
                  @click.stop="newClass(selected)"
                  absolute
                  bottom
                  color="white"
                  fab
              ...
              <v-list>
                <v-list-item-group>
                  <v-list-item
                    v-for="clss in selected.classes"
                    :key="clss.id"
                    @click="test(clss._id)"
                  >
                    <v-list-item-icon>
          ...
          <v-card
            v-else-if="nodeTypeMap[selected.id] == 'class'"
            class="pt-6"
            flat
          >
            <v-card-text class="text-center">
              ...
              <v-divider></v-divider>
              <h4 class="subtitle-1 my-5">
                {{ selected.sessions.length }} sessions enregistrées
              </h4>
              <v-toolbar color="#46c4a1" dark dense>
                <v-toolbar-title class="px-3">Sessions</v-toolbar-title>
                <v-btn
                  @click.stop="newSession(selected)"
                  absolute
                  bottom
                  color="white"
                  fab
              ...
              <v-list>
                <v-list-item-group>
                  <v-list-item
                    v-for="sess in selected.sessions"
                    :key="sess.id"
                    @click="test(sess._id)"
                  >
                    <v-list-item-icon>
                      <v-img max-height="50" contain src="@/assets/map.svg">
                      </v-img>
                    </v-list-item-icon>
                    <v-list-item-content>
                      {{ sess.session_name }}
          ...
          <v-card
            v-else-if="nodeTypeMap[selected.id] == 'session'"
            class="pt-6"
            flat
          >
            ...
            <v-row>
              ...
              <v-col>
                <v-card-text class="text-center">
                  ...
                  <v-divider></v-divider>
                  <h4 class="subtitle-1 my-5">
                    {{ selected.beacons.length }} Balises enregistrées
                  </h4>
                  <h4 class="subtitle-1 my-5">
                    {{ selected.runs.length }} Éleves enregistrées
                  </h4>
                </v-card-text>
              </v-col>
            </v-row>
          ...
          <v-card
            v-else-if="nodeTypeMap[selected.id] == 'run'"
            class="pt-6"
            flat
          >
            run
...
export default {
  ...
  methods: {
      ...
      switch (this.nodeTypeMap[parent.id]) {
        ...
        case "class":
          ...
          for (const session of res.data.sessions) {
            let sessionInfo = (await axios.get(`/api/sessions/${session.id}`))
              .data;
            ...
            console.log("session.runs", session);
            this.itemsFlat.push(item);
            parent.children.push(item);
          }
          break;
    ...
    async newClassSendPost() {
      ...
      const res = await axios.post(
        `/api/schools/${this.newItem.parent.id}`,
        this.newItem
      );
      let item = {
        ...this.newItem,
      ...
      this.itemsFlat
        .find((e) => (e.id = this.newItem.parent.id))
        .classes.push(item);
      this.itemsFlat
        .find((e) => (e.id = this.newItem.parent.id))
        .children.push(item);
    ...
    async newSessionSendPost() {
      ...
      item.id = res.data.id;
      console.log("parent", this.newItem.parent);
      this.newItem.parent.sessions.push(item);
      this.newItem.parent.children.push(item);
      this.itemsFlat.push(item);
      this.nodeTypeMap[item.id] = "session";
      console.log(this.newItem);
      //const res = await axios.post(`/api/sessions/`,newSession, {params: {class_id: this.newItem.parent.id}});
      // console.log(res);
    },
    ...
    async saveNew() {
      ...
      try {
        ...
        this.selected.beacons.push({id: this.editedItem.id, coords: [this.editedItem.lat, this.editedItem.long]});
        this.axiosSuccess = true;
  ...
  computed: {
    selected() {
      ...
      console.log("returning",this.itemsFlat.find((element) => element.id == this.active[0]));
    ...
    formattedDate() {
      return this.newItem.date
        ? moment(this.newItem.date).format("dddd Do MMMM")
        : "";
    },
    beacons() {
                console.log("here")
  ...
  watch: {
    selected(val) {
      if (this.nodeTypeMap[val.id] == "run") {
        console.log(
          "redirect",
          `/session/${val.parent.id}/student-recap/${val.name}`
        );
        this.$router.push(
          `/session/${val.parent.id}/student-recap/${val.name}`
        );
